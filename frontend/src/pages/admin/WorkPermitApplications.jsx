...
const AdminWorkPermitApplications = () => {
  ...
  const fetchApplications = useCallback(async () => {
    try {
      ...
      const url = `/work-permit-applications${params.toString() ? '?' + params.toString() : ''}`
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await api.patch(`/work-permit-applications/${selectedApplication.id}/status`, {
        status,
        notes,
  ...
  const handleNotifyClient = async (application) => {
    try {
      await api.post(`/work-permit-applications/${application.id}/notify-client`)
  ...
  return (
    <Layout>
      <div className="section-container py-8 lg:py-12">
              ...
              <div className="p-4 sm:p-6 lg:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <Card padding="lg" className="bg-gradient-to-br from-accent-50 to-transparent border-accent-100">
                    ...
                    <p className="text-lg font-bold text-neutral-900">{applicationDetails.country?.name || 'Pays inconnu'}</p>
                    {applicationDetails.country?.subtitle && (
                      <p className="text-sm text-neutral-600 mt-1">{applicationDetails.country.subtitle}</p>
                    )}
                  </Card>
                  ...
                  {(applicationDetails.age || applicationDetails.profession || applicationDetails.experience_years || applicationDetails.current_employer || applicationDetails.phone_number || applicationDetails.address || applicationDetails.education_level || applicationDetails.language_skills) && (
                    <div className="col-span-1 md:col-span-2">
                      <Card padding="lg" className="bg-purple-50/50 border-purple-200">
                        <h3 className="text-xs font-semibold text-purple-700 uppercase tracking-wider mb-3">Informations personnelles</h3>
                        <div className="grid md:grid-cols-2 gap-4">
                          {applicationDetails.age && (
                            <div>
                              <p className="text-xs text-neutral-600 mb-1">Âge</p>
                              <p className="text-sm font-semibold text-neutral-900">{applicationDetails.age} ans</p>
                            </div>
                          )}
                          {applicationDetails.profession && (
                            <div>
                              <p className="text-xs text-neutral-600 mb-1">Profession</p>
                              <p className="text-sm font-semibold text-neutral-900">{applicationDetails.profession}</p>
                            </div>
                          )}
                          {applicationDetails.experience_years && (
                            <div>
                              <p className="text-xs text-neutral-600 mb-1">Années d'expérience</p>
                              <p className="text-sm font-semibold text-neutral-900">{applicationDetails.experience_years} ans</p>
                            </div>
                          )}
                          {applicationDetails.current_employer && (
                            <div>
                              <p className="text-xs text-neutral-600 mb-1">Employeur actuel</p>
                              <p className="text-sm font-semibold text-neutral-900">{applicationDetails.current_employer}</p>
                            </div>
                          )}
                          {applicationDetails.phone_number && (
                            <div>
                              <p className="text-xs text-neutral-600 mb-1">Numéro de téléphone</p>
                              <p className="text-sm font-semibold text-neutral-900">{applicationDetails.phone_number}</p>
                            </div>
                          )}
                          {applicationDetails.education_level && (
                            <div>
                              <p className="text-xs text-neutral-600 mb-1">Niveau d'éducation</p>
                              <p className="text-sm font-semibold text-neutral-900">{applicationDetails.education_level}</p>
                            </div>
                          )}
                        </div>
                        {applicationDetails.address && (
                          <div className="mt-4">
                            <p className="text-xs text-neutral-600 mb-1">Adresse</p>
                            <p className="text-sm font-semibold text-neutral-900">{applicationDetails.address}</p>
                          </div>
                        )}
                        {applicationDetails.language_skills && (
                          <div className="mt-4">
                            <p className="text-xs text-neutral-600 mb-1">Compétences linguistiques</p>
                            <p className="text-sm font-semibold text-neutral-900">{applicationDetails.language_skills}</p>
                          </div>
                        )}
                      </Card>
                    </div>
        ...
        {/* Modals d'édition, rejet et prévisualisation - identiques à ResidenceApplications */}
...
export default AdminWorkPermitApplications
