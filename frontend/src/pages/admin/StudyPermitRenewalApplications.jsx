...
import { FiCheckCircle, FiXCircle, FiClock, FiFile, FiDownload, FiEye, FiX, FiSearch, FiFilter, FiX as FiXIcon, FiFileText, FiUser, FiBell, FiAlertCircle, FiArrowRight, FiTrash2, FiMapPin, FiCalendar, FiBook } from 'react-icons/fi'
import { Link } from 'react-router-dom'

const AdminStudyPermitRenewalApplications = () => {
  ...
  const fetchApplications = useCallback(async () => {
    try {
      ...
      const url = `/study-permit-renewal-applications${params.toString() ? '?' + params.toString() : ''}`
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      await api.patch(`/study-permit-renewal-applications/${selectedApplication.id}/status`, {
        status,
        notes,
  ...
  const handleNotifyClient = async (application) => {
    try {
      await api.post(`/study-permit-renewal-applications/${application.id}/notify-client`)
  ...
  const handleDeleteApplication = async () => {
    ...
    try {
      await api.delete(`/study-permit-renewal-applications/${applicationToDelete.id}`)
      toast.success('Demande de renouvellement supprimée avec succès')
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-neutral-600">
              Gérez toutes les demandes de renouvellement de CAQ ou de permis d'études
            </p>
          </div>
        ...
        <div className="grid gap-6">
          {applications.length === 0 ? (
            <Card className="p-12 text-center">
              ...
              <p className="text-neutral-600 text-lg">Aucune demande de renouvellement</p>
            </Card>
          ) : (
              ...
              <div className="p-4 sm:p-6 lg:p-8">
                <div className="flex justify-between items-start mb-4 sm:mb-6 gap-4">
                  <div className="flex-1 min-w-0">
                    <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-neutral-900 mb-1">
                      Détails de la demande de renouvellement
                    </h2>
                    <p className="text-xs sm:text-sm text-neutral-500">CAQ/Permis d'études - {applicationDetails.country || 'Canada'}</p>
                  </div>
                  <button
                    onClick={() => {
                      setShowDetailsModal(false)
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <Card padding="lg" className="bg-gradient-to-br from-accent-50 to-transparent border-accent-100">
                    ...
                    <p className="text-lg font-bold text-neutral-900">{applicationDetails.country || 'Canada'}</p>
                  </Card>
                  {applicationDetails.arrival_date && (
                    <Card padding="lg" className="bg-blue-50 border-blue-100">
                      <h3 className="text-xs font-semibold text-blue-600 uppercase tracking-wider mb-2">Date d'arrivée au Canada</h3>
                      <p className="text-lg font-semibold text-neutral-900 mt-1">
                        {new Date(applicationDetails.arrival_date).toLocaleDateString('fr-FR', {
                          day: 'numeric',
                          month: 'long',
                          year: 'numeric',
                        })}
                  ...
                  {applicationDetails.institution_name && (
                    <Card padding="lg" className="bg-green-50 border-green-100">
                      <h3 className="text-xs font-semibold text-green-600 uppercase tracking-wider mb-2">Établissement</h3>
                      <p className="text-lg font-semibold text-neutral-900 mt-1">{applicationDetails.institution_name}</p>
                    </Card>
                  )}
                  {applicationDetails.expiration_date && (
                    <Card padding="lg" className="bg-orange-50 border-orange-100">
                      <h3 className="text-xs font-semibold text-orange-600 uppercase tracking-wider mb-2">Date d'expiration</h3>
                      <p className="text-lg font-semibold text-neutral-900 mt-1">
                        {new Date(applicationDetails.expiration_date).toLocaleDateString('fr-FR', {
                          day: 'numeric',
                          month: 'long',
                          year: 'numeric',
                        })}
                  ...
                  {(applicationDetails.address || applicationDetails.address_number) && (
                    <Card padding="lg" className="bg-indigo-50 border-indigo-100 md:col-span-2">
                      <h3 className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-2">Adresse domicile</h3>
                      <p className="text-sm font-semibold text-neutral-900 mt-1">
                        {applicationDetails.address_number && `${applicationDetails.address_number} `}
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-4 flex-1 min-w-0">
                                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-accent-500 to-accent-800 flex items-center justify-center flex-shrink-0 shadow-md">
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
            <Card className="max-w-md w-full p-8 animate-scale-in">
              ...
              <p className="text-gray-600 mb-4">
                Êtes-vous sûr de vouloir supprimer la demande de renouvellement du client <span className="font-semibold">{applicationToDelete.user?.name || 'inconnu'}</span> ?
              </p>
...
export default AdminStudyPermitRenewalApplications
